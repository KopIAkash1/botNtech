...
def help(message):
    bot.send_message(message.chat.id,
                     '''/start - запуск экземпляра для уведомлений о новых, неназначенных никому тикетов \
...
def info_config(message):
    ...
    elif str(message.text).split( )[1] == "no_reply":
    ...
    elif str(message.text).split( )[1] == "reply_time":
